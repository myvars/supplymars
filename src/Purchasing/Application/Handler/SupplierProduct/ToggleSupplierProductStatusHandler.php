...
final readonly class ToggleSupplierProductStatusHandler
{
    ...
    public function __invoke(ToggleSupplierProductStatus $command): Result
    {
        ...
        $supplierProduct->setActive(!$supplierProduct->isActive());

        $this->flusher->flush();

        return Result::ok('Supplier product status changed');
    }
}
